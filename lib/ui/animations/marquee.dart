...
class Marquee extends StatefulWidget {
  ...
  final TextStyle? styleDelemitter;
...
class _MarqueeState extends State<Marquee> with SingleTickerProviderStateMixin {
  ...
  double? _accelerationTarget;
  double? _linearTarget;
  double? _decelerationTarget;
  ...
  bool get showFading =>
      !widget.showFadingOnlyWhenScrolling ? !widget.showFadingOnlyWhenScrolling : !_isOnPause;
  ...
  double _getTextWidth(BuildContext context) {
    final widgetText = widget.text + widget.textPrefix!;
    final span = TextSpan(text: widgetText, style: widget.styleText);
    ...
    final boxes = renderObject.getBoxesForSelection(TextSelection(
      baseOffset: 0,
      extentOffset: TextSpan(text: widgetText).toPlainText().length,
    ));
  ...
  Widget build(BuildContext context) {
    _initialize(context);
    final bool isHorizontal = widget.scrollAxis == Axis.horizontal;
    ...
    final Widget marquee = ListView.builder(
      ...
      itemBuilder: (_, i) {
        return getTextWidgets(i, alignment, context);
      },
    );
  ...
  Widget getTextWidgets(int i, Alignment? alignment, BuildContext context) {
    return i.isEven
        ? SizedBox(
            child: Row(
              children: [
                ...
                if (widget.textPrefix != null) ...[
                  Text(
                    '  -  ',
                    style: widget.styleDelemitter,
                  ),
                  Text(
                    widget.textPrefix!,
                    style: widget.stylePrefix,
                  ),
                  const Text('      '),
                ],
              ],
